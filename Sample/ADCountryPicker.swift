...
@objc public protocol ADCountryPickerDelegate: class {
    ...
    func countryPicker(_ picker: ADCountryPicker,
                                      didSelectCountryWithName name: String,
                                      code: String,
                                      dialCode: String,flagImg: UIImage)
}
...
open class ADCountryPicker: UITableViewController {
    ...
    fileprivate var unsortedCountries : [ADCountry] {
        ...
        for countryCode in countriesCodes {
            ...
            if countryData.count > 0, let dialCode = countryData[0]["dial_code"] {
                country = ADCountry(name: displayName!, code: countryCode, dialCode: dialCode, flagImg: nil)
            } else {
                country = ADCountry(name: displayName!, code: countryCode, flagImg: nil)
            }
    ...
    fileprivate var sections: [Section] {
        ...
        let countries: [ADCountry] = unsortedCountries.map { country in
            let country = ADCountry(name: country.name, code: country.code, dialCode: country.dialCode, flagImg: nil)
        ...
        for section in sections {
            var s = section
             let str = #selector(getter: ADCountry.name)
            s.countries = collation.sortedArray(from: section.countries, collationStringSelector: str) as! [ADCountry]
        }
...
extension ADCountryPicker {
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var tempCell: UITableViewCell? = tableView.dequeueReusableCell(withIdentifier: UITableViewCell.identifier)
        
        if tempCell == nil {
            tempCell = UITableViewCell(style: .default, reuseIdentifier: UITableViewCell.identifier)
        }
...
extension ADCountryPicker {
    ...
    override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let bundle = "assets.bundle/"
        var imgaeFlag = UIImage()
        
        if self.showFlags == true {
            let image = UIImage(named: bundle + country.code.uppercased() + ".png", in: Bundle(for: ADCountryPicker.self), compatibleWith: nil)
            if (image != nil) {
                imgaeFlag = (image?.fitImage(size: CGSize(width:self.flagHeight, height:flagHeight)))!
            }
            else {
                imgaeFlag = (UIImage.init(color: .lightGray,
                                          size: CGSize(width:CGFloat(flagHeight), height:CGFloat(flagHeight)/CGFloat(1.5)))?.fitImage(size: CGSize(width:CGFloat(self.flagHeight), height:CGFloat(flagHeight)/CGFloat(1.5))))!
            }
        }
        ...
        delegate?.countryPicker(self, didSelectCountryWithName: country.name, code: country.code, dialCode: country.dialCode, flagImg: imgaeFlag)
